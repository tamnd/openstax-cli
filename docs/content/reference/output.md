...
describes what users get. Pick a format with `-o`, or let openstax choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
openstax <command> -o table   # aligned columns for reading
openstax <command> -o jsonl   # one JSON object per line, for piping
openstax <command> -o json    # a single JSON array
openstax <command> -o csv     # spreadsheet friendly
openstax <command> -o tsv     # tab-separated
openstax <command> -o url     # just the URL column
openstax <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
openstax <command> --fields id,title,url
```
...
```bash
openstax <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
openstax <command>            # a table, because this is a terminal
openstax <command> | wc -l    # JSONL, because this is a pipe
```
