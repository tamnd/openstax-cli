// Package cli builds the openstax command tree on top of the openstax library.
package cli
...
// App holds shared state for the command tree.
type App struct {
	client *openstax.Client
	cfg    openstax.Config
...
func Root() *cobra.Command {
	app := &App{cfg: openstax.DefaultConfig()}

	root := &cobra.Command{
		Use:           "openstax",
		Short:         "Browse free OpenStax textbooks",
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = openstax.NewClient(a.cfg)
	return nil
}
